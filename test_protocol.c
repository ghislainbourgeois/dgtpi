...
static int mock_i2c_send(const uint8_t *message, uint8_t length, uint8_t ack_address) {
...
static int mock_i2c_receive_ready(void) {
    return mock_state.receive_count > 0;
}

static int mock_i2c_receive(uint8_t *buffer, uint8_t max_length) {
    if (mock_state.receive_count == 0) return 0;
    // Return mock message
    buffer[0] = 0x50;
    buffer[1] = 0x10;
    buffer[2] = 0x05;
...
static uint64_t mock_get_timer_us(void) {
    return 0;
}

static int mock_check_core_freq_mhz(void) {
    return 250;
}
