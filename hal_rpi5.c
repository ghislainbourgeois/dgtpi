...
static int hal_rpi5_map_memory(void) {
    ...
    i2c1_regs = mmap(NULL, 4096, PROT_READ|PROT_WRITE, MAP_SHARED, memfd, RPI5_I2C1_BASE);
    timer64 = mmap(NULL, 4096, PROT_READ|PROT_WRITE, MAP_SHARED, memfd, RPI5_TIMER_BASE);
...
static uint32_t hal_rpi5_i2c1_read(uint8_t reg) {
    return i2c1_regs[reg/4];
}
...
static int hal_rpi5_i2c_send(const uint8_t *message, uint8_t length, uint8_t ack_address) {
    int i, n;
    uint64_t timeOut;
    char m[256];
    ...
    for (n=1; n<m[2] && (hal_rpi5_i2c1_read(RPI5_I2C1_STAT)&0x10)==0; n++) {
    ...
    for (i=0; i<256; i++) {
        if (i == 0) i = -1;
        if (hal.get_timer_us() > timeOut) return -6;
    }
    ...
    for (; n<m[2]; n++) {
        ...
        while ((hal_rpi5_i2c1_read(RPI5_I2C1_STAT)&0x10)==0) {
            if (hal_rpi5_i2c1_read(RPI5_I2C1_STAT)&2) break;
            if (hal.get_timer_us() > timeOut) return -6;
        }
        if (hal_rpi5_i2c1_read(RPI5_I2C1_STAT)&2) break;
        hal_rpi5_i2c1_write(RPI5_I2C1_FIFO, m[n]);
    ...
    if (stat & 0x100) return -2;
    if (stat & 0x200) return -4;
...
static void hal_rpi5_stop_hardware(void) {
    if (i2c1_regs) munmap((void*)i2c1_regs, 4096);
    if (timer64) munmap((void*)timer64, 4096);
    i2c1_regs = NULL;
    timer64 = NULL;
}
...
int hal_rpi5_init(void) {
    if (hal_rpi5_map_memory() < 0) return -1;
    hal_rpi5_config_i2c1_pins();
    return 0;
}

void hal_rpi5_cleanup(void) {
    hal_rpi5_stop_hardware();
}

const char* hal_rpi5_get_platform_name(void) {
